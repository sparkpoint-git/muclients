...
final class FLTheme {
	...
	static public function enqueue_scripts() {
		...
		if ( 'file' === FLTheme::get_asset_enqueue_method() ) {
			...
			wp_register_style( 'fl-automator-skin', false );
	...
	static public function logo() {
		...
		if ( 'image' === $logo_type ) {
			...
			if ( $mobile_logo ) {
				$mobile_data = wp_get_attachment_metadata( attachment_url_to_postid( $mobile_logo ) );
	...
	static public function comment_form_default_fields( $fields ) {
		...
		$fields['author'] = '<label for="fl-author">' . _x( 'Name', 'Comment form label: comment author name.', 'fl-automator' ) . ( $req ? __( ' (required)', 'fl-automator' ) : '' ) . '</label>
									<input type="text" id="fl-author" name="author" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '"' . ( $req ? ' aria-required="true"' : '' ) . ' /><br />';
		...
		$fields['email'] = '<label for="fl-email">' . _x( 'Email (will not be published)', 'Comment form label: comment author email.', 'fl-automator' ) . ( $req ? __( ' (required)', 'fl-automator' ) : '' ) . '</label>
									<input type="text" id="fl-email" name="email" class="form-control" value="' . esc_attr( $commenter['comment_author_email'] ) . '"' . ( $req ? ' aria-required="true"' : '' ) . ' /><br />';

		$fields['url'] = '<label for="fl-url">' . _x( 'Website', 'Comment form label: comment author website.', 'fl-automator' ) . '</label>
									<input type="text" id="fl-url" name="url" class="form-control" value="' . esc_attr( $commenter['comment_author_url'] ) . '" /><br />';
	...
	public static function comment_form_defaults( $args ) {
		...
		$args = array(
			...
			'comment_field'        => '<label for="fl-comment">' . _x( 'Comment', 'Comment form label: comment content.', 'fl-automator' ) . '</label><textarea id="fl-comment" name="comment" class="form-control" cols="60" rows="8"></textarea><br />',
