	...
	if ( count( $settings->screens ) > 0 ) {
		...
		<li class="uabb-creative-link uabb-cl-<?php echo esc_attr( $settings->link_style ); ?> <?php echo esc_attr( $current_class ); ?>">
			<<?php echo esc_attr( $settings->link_typography_tag_selection ); ?> class="uabb-cl-heading">
				<a href="<?php echo esc_url( $screen_link ); ?>" target="<?php echo esc_attr( $target ); ?>" <?php BB_Ultimate_Addon_Helper::get_link_rel( $target, $nofollow, 1 ); ?> data-hover="<?php echo esc_attr( $screen->title ); ?>" aria-label="<?php echo esc_attr( $screen->title ); ?>"><?php $module->render_text( $screen->title ); ?></a>
