...
class ServiceFactory
{
    ...
    public function getServices(\Mpdf\Mpdf $mpdf, LoggerInterface $logger, $config, $languageToFont, $scriptToLanguage, $fontDescriptor, $bmp, $directWrite, $wmf)
    {
        $sizeConverter = new \Mpdf\SizeConverter($mpdf->dpi, $mpdf->default_font_size, $mpdf, $logger);
        ...
        $tableOfContents = new \Mpdf\TableOfContents($mpdf, $sizeConverter);
        $cacheBasePath = $config['tempDir'] . '/mpdf';
        $cache = new \Mpdf\Cache($cacheBasePath, $config['cacheCleanupInterval']);
        $fontCache = new FontCache(new \Mpdf\Cache($cacheBasePath . '/ttfontdata', $config['cacheCleanupInterval']));
        ...
        $assetFetcher = new \Mpdf\AssetFetcher($mpdf, $localContentLoader, $httpClient, $logger);
        $cssManager = new \Mpdf\CssManager($mpdf, $cache, $sizeConverter, $colorConverter, $assetFetcher);
        $otl = new \Mpdf\Otl($mpdf, $fontCache);
        $protection = new Protection(new UniqidGenerator());
        $writer = new BaseWriter($mpdf, $protection);
        $gradient = new \Mpdf\Gradient($mpdf, $sizeConverter, $colorConverter, $writer);
        $formWriter = new FormWriter($mpdf, $writer);
        $form = new \Mpdf\Form($mpdf, $otl, $colorConverter, $writer, $formWriter);
        $hyphenator = new \Mpdf\Hyphenator($mpdf);
        ...
        $tag = new \Mpdf\Tag($mpdf, $cache, $cssManager, $form, $otl, $tableOfContents, $sizeConverter, $colorConverter, $imageProcessor, $languageToFont);
