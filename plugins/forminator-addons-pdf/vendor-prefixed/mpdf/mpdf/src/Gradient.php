...
class Gradient
{
     ...
     */
    private $writer;
    public function __construct(\Mpdf\Mpdf $mpdf, \Mpdf\SizeConverter $sizeConverter, ColorConverter $colorConverter, BaseWriter $writer)
    {
        $this->mpdf = $mpdf;
    ...
    {
        $s = ' q ';
        $s .= \sprintf(' %.3F %.3F %.3F %.3F re W n ', $x * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - $y) * \Mpdf\Mpdf::SCALE, $w * \Mpdf\Mpdf::SCALE, -$h * \Mpdf\Mpdf::SCALE);
        $s .= \sprintf(' %.3F 0 0 %.3F %.3F %.3F cm ', $w * \Mpdf\Mpdf::SCALE, $h * \Mpdf\Mpdf::SCALE, $x * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($y + $h)) * \Mpdf\Mpdf::SCALE);
    ...
    public function Gradient($x, $y, $w, $h, $type, $stops = [], $colorspace = 'RGB', $coords = '', $extend = '', $return = \false, $is_mask = \false)
    {
        ...
        $s = ' q';
        $s .= \sprintf(' %.3F %.3F %.3F %.3F re W n', $x * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - $y) * \Mpdf\Mpdf::SCALE, $w * \Mpdf\Mpdf::SCALE, -$h * \Mpdf\Mpdf::SCALE) . "\n";
        $s .= \sprintf(' %.3F 0 0 %.3F %.3F %.3F cm', $usew * \Mpdf\Mpdf::SCALE, $useh * \Mpdf\Mpdf::SCALE, $usex * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($usey + $useh)) * \Mpdf\Mpdf::SCALE) . "\n";
        $n = \count($this->mpdf->gradients) + 1;
