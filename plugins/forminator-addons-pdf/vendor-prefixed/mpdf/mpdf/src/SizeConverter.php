...
class SizeConverter implements \ForminatorPDFAddon\Psr\Log\LoggerAwareInterface
{
     ...
     */
    private $mpdf;
    public function __construct($dpi, $defaultFontSize, \Mpdf\Mpdf $mpdf, LoggerInterface $logger)
    {
        $this->dpi = $dpi;
    ...
    public function convert($size = 5, $maxsize = 0, $fontsize = \false, $usefontsize = \true)
    {
        ...
        switch ($unit) {
            ...
            case 'cm':
                ...
                break;
            case 'pt':
                $size *= 1 / \Mpdf\Mpdf::SCALE;
                break;
            case 'rem':
                $size *= $this->mpdf->default_font_size / \Mpdf\Mpdf::SCALE;
                break;
            case '%':
