...
class Form
{
    ...
    public function __construct(\Mpdf\Mpdf $mpdf, \Mpdf\Otl $otl, ColorConverter $colorConverter, BaseWriter $writer, FormWriter $formWriter)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
    ...
    function print_ob_textarea($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $ClipPath = \sprintf('q %.3F %.3F %.3F %.3F re W n ', $this->mpdf->x * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - $this->mpdf->y) * \Mpdf\Mpdf::SCALE, $w * \Mpdf\Mpdf::SCALE, -$h * \Mpdf\Mpdf::SCALE);
    ...
    function print_ob_imageinput($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir, $is_table)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            $this->writer->write(\sprintf('q %.3F 0 0 %.3F %.3F %.3F cm /I%d Do Q', $objattr['INNER-WIDTH'] * \Mpdf\Mpdf::SCALE, $objattr['INNER-HEIGHT'] * \Mpdf\Mpdf::SCALE, $objattr['INNER-X'] * \Mpdf\Mpdf::SCALE, ($this->mpdf->h - ($objattr['INNER-Y'] + $objattr['INNER-HEIGHT'])) * \Mpdf\Mpdf::SCALE, $objattr['ID']));
    ...
    {
        $x *= \Mpdf\Mpdf::SCALE;
        $y = $hPt - $y * \Mpdf\Mpdf::SCALE;
        $x2 = $x + $w * \Mpdf\Mpdf::SCALE;
        $y2 = $y - $h * \Mpdf\Mpdf::SCALE;
