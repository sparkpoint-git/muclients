...
final class ResourceWriter implements \ForminatorPDFAddon\Psr\Log\LoggerAwareInterface
{
    use Strict;
    ...
    public function __construct(Mpdf $mpdf, \Mpdf\Writer\BaseWriter $writer, \Mpdf\Writer\ColorWriter $colorWriter, \Mpdf\Writer\FontWriter $fontWriter, \Mpdf\Writer\ImageWriter $imageWriter, \Mpdf\Writer\FormWriter $formWriter, \Mpdf\Writer\OptionalContentWriter $optionalContentWriter, \Mpdf\Writer\BackgroundWriter $backgroundWriter, \Mpdf\Writer\BookmarkWriter $bookmarkWriter, \Mpdf\Writer\MetadataWriter $metadataWriter, \Mpdf\Writer\JavaScriptWriter $javaScriptWriter, LoggerInterface $logger)
    {
        $this->mpdf = $mpdf;
        $this->writer = $writer;
