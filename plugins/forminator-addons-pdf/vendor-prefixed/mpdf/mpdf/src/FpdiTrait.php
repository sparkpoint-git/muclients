...
use ForminatorPDFAddon\setasign\Fpdi\PdfParser\Filter\AsciiHex;
...
trait FpdiTrait
{
    use \ForminatorPDFAddon\setasign\Fpdi\FpdiTrait {
        writePdfType as fpdiWritePdfType;
        useImportedPage as fpdiUseImportedPage;
        importPage as fpdiImportPage;
    }
    protected $k = \Mpdf\Mpdf::SCALE;
    /**
        ...
        if ($annotations instanceof PdfArray) {
            ...
            foreach ($annotations->value as $annotation) {
                try {
                    ...
                    $links[] = ['x' => $rect->getLlx() / \Mpdf\Mpdf::SCALE, 'y' => $rect->getLly() / \Mpdf\Mpdf::SCALE, 'width' => $rect->getWidth() / \Mpdf\Mpdf::SCALE, 'height' => $rect->getHeight() / \Mpdf\Mpdf::SCALE, 'url' => $uri->value];
