...
class CssManager
{
    ...
    public function __construct(\Mpdf\Mpdf $mpdf, \Mpdf\Cache $cache, \Mpdf\SizeConverter $sizeConverter, ColorConverter $colorConverter, \Mpdf\AssetFetcher $assetFetcher)
    {
        $this->mpdf = $mpdf;
        $this->cache = $cache;
            ...
            if ($k === 'FONT') {
                ...
                if (\count($prop) == 2) {
                    ...
                    if (\preg_match('/([0-9a-zA-Z]*)-L/i', $v, $m)) {
                        ...
                        $ft = \Mpdf\PageFormat::getSizeFromName($m[1]);
                        $format = [$ft[1], $ft[0]];
                    } else {
                        $format = \Mpdf\PageFormat::getSizeFromName($v);
                    }
                    if ($format) {
                        $newprop['SHEET-SIZE'] = [$format[0] / \Mpdf\Mpdf::SCALE, $format[1] / \Mpdf\Mpdf::SCALE];
                    }
                }
            } elseif ($k === 'BACKGROUND') {
