...
class ImageProcessor implements \ForminatorPDFAddon\Psr\Log\LoggerAwareInterface
{
    ...
    public function __construct(Mpdf $mpdf, Otl $otl, CssManager $cssManager, SizeConverter $sizeConverter, ColorConverter $colorConverter, ColorModeConverter $colorModeConverter, Cache $cache, LanguageToFontInterface $languageToFont, ScriptToLanguageInterface $scriptToLanguage, AssetFetcher $assetFetcher, LoggerInterface $logger)
    {
        ...
        $this->logger = $logger;
        $this->guesser = new \Mpdf\Image\ImageTypeGuesser();
        $this->failedImages = [];
    }
    ...
    {
        $svg = new \Mpdf\Image\Svg($this->mpdf, $this->otl, $this->cssManager, $this, $this->sizeConverter, $this->colorConverter, $this->languageToFont, $this->scriptToLanguage);
    ...
    {
        if ($this->bmp === null) {
            $this->bmp = new \Mpdf\Image\Bmp($this->mpdf);
        }
    ...
    {
        if ($this->wmf === null) {
            $this->wmf = new \Mpdf\Image\Wmf($this->mpdf, $this->colorConverter);
        }
