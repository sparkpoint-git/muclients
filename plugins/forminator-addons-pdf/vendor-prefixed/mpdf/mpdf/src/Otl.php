...
class Otl
{
    ...
    var $debugOTL = \false;
    public function __construct(\Mpdf\Mpdf $mpdf, FontCache $fontCache)
    {
        $this->mpdf = $mpdf;
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        foreach ($earr as $char) {
            $ucd_record = \Mpdf\Ucdn::get_ucd_record($char);
            $sbl = $ucd_record[6];
            // Special case - Arabic End of Ayah
            if ($char == 1757) {
                $sbl = \Mpdf\Ucdn::SCRIPT_ARABIC;
            }
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            ...
            if (\Mpdf\Ucdn::SCRIPT_DEVANAGARI <= $scriptblock && $scriptblock <= \Mpdf\Ucdn::SCRIPT_MALAYALAM) {
                $this->shaper = "I";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_ARABIC || $scriptblock == \Mpdf\Ucdn::SCRIPT_SYRIAC) {
                $this->shaper = "A";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_NKO || $scriptblock == \Mpdf\Ucdn::SCRIPT_MANDAIC) {
                $this->shaper = "A";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_KHMER) {
                $this->shaper = "K";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_THAI) {
                $this->shaper = "T";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_LAO) {
                $this->shaper = "L";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_SINHALA) {
                $this->shaper = "S";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_MYANMAR) {
                $this->shaper = "M";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_NEW_TAI_LUE) {
                $this->shaper = "E";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_CHAM) {
                $this->shaper = "E";
            } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_TAI_THAM) {
                $this->shaper = "E";
            } else {
                $this->shaper = "";
            }
            // Get scripttag based on actual text script
            $scripttag = \Mpdf\Ucdn::$uni_scriptblock[$scriptblock];
            $GSUBscriptTag = '';
            $GSUBlangsys = '';
            $GPOSscriptTag = '';
            $GPOSlangsys = '';
            ...
            } elseif ($this->mpdf->useTibetanLBR && $scriptblock == \Mpdf\Ucdn::SCRIPT_TIBETAN) {
                // Sets $this->OTLdata[$i]['wordend']=true at possible end of word boundaries
            ...
            if ($useOTL & 0xff && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
                ...
                if ($this->shaper == 'A') {
                    ...
                    for ($ptr = 1; $ptr < \count($this->OTLdata); $ptr++) {
                        if ($this->OTLdata[$ptr]['general_category'] == \Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK && $this->OTLdata[$ptr - 1]['uni'] == 32) {
                    ...
                    for ($ptr = 0; $ptr < \count($this->OTLdata); $ptr++) {
                        ...
                        if ($sub) {
                            ...
                            for ($i = 0; $i < \count($sub); $i++) {
                                $newinfo[$i] = [];
                                $ucd_record = \Mpdf\Ucdn::get_ucd_record($sub[$i]);
                        ...
                        if ($this->shaper == 'I') {
                            if ($char == 0x9af && isset($this->OTLdata[$ptr + 1]) && $this->OTLdata[$ptr + 1]['uni'] == 0x9bc) {
                                ...
                                $newinfo = [];
                                $newinfo[0] = [];
                                $ucd_record = \Mpdf\Ucdn::get_ucd_record($sub);
                    ...
                    if ($broken_syllables) {
                        if ($this->mpdf->_charDefined($this->mpdf->fonts[$this->fontkey]['cw'], 0x25cc)) {
                            $dottedcircle = [];
                            $ucd_record = \Mpdf\Ucdn::get_ucd_record(0x25cc);
                    ...
                    if ($this->shaper == 'T' || $this->shaper == 'L') {
                        for ($ptr = 0; $ptr < \count($this->OTLdata); $ptr++) {
                            ...
                            if (($char & ~0x80) == 0xe33) {
                                ...
                                $newinfo = [];
                                $ucd_record = \Mpdf\Ucdn::get_ucd_record($sub[0]);
                                $newinfo[0]['general_category'] = $ucd_record[0];
                                $newinfo[0]['bidi_type'] = $ucd_record[2];
                                ...
                                $newinfo = [];
                                $ucd_record = \Mpdf\Ucdn::get_ucd_record($sub[1]);
                                $newinfo[0]['general_category'] = $ucd_record[0];
                                $newinfo[0]['bidi_type'] = $ucd_record[2];
                    ...
                    if ($scriptblock == \Mpdf\Ucdn::SCRIPT_TIBETAN) {
                        // =========================
                        // Reordering TIBETAN
                        // =========================
                    ...
                    if (!($this->mpdf->textvar & TextVars::FC_SMALLCAPS && $scriptblock == \Mpdf\Ucdn::SCRIPT_LATIN && \strpos($useGSUBtags, 'smcp') === \false)) {
            ...
            //=======================================================
            if ($this->shaper == "K" || $this->shaper == "T" || $this->shaper == "L" || $scriptblock == \Mpdf\Ucdn::SCRIPT_TIBETAN) {
                ...
                $newinfo[0] = ['general_category' => \Mpdf\Ucdn::UNICODE_GENERAL_CATEGORY_FORMAT, 'bidi_type' => \Mpdf\Ucdn::BIDI_CLASS_BN, 'group' => 'S', 'uni' => 0x200b, 'hex' => '0200B'];
            ...
            if ($useOTL & 0xff && $GPOSscriptTag && $GPOSlangsys && $GPOSFeatures) {
                ...
                if ($scriptblock != \Mpdf\Ucdn::SCRIPT_LATIN) {
                    $tags .= ' kern';
                }
                $omittags = '';
                $usetags = $tags;
    ...
    public function bidiSort($ta, $str, $dir, &$chunkOTLdata, $useGPOS)
    {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            ...
            $chardata[$i]['sor'] = \max($left, $level) % 2 ? \Mpdf\Ucdn::BIDI_CLASS_R : \Mpdf\Ucdn::BIDI_CLASS_L;
            $chardata[$i]['eor'] = \max($right, $level) % 2 ? \Mpdf\Ucdn::BIDI_CLASS_R : \Mpdf\Ucdn::BIDI_CLASS_L;
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_NSM) {
        ...
        $levcount = 0;
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                $found = \false;
                for ($j = $levcount; $j >= 0; $j--) {
                    if ($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                        $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_AN;
                        $found = \true;
                        break;
                    } elseif ($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                        $found = \true;
                        break;
                    }
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_R;
            }
        }
        ...
        for ($i = 1; $i < $numchars; ++$i) {
            if ($i + 1 < $numchars && $chardata[$i]['level'] == $chardata[$i + 1]['level'] && $chardata[$i]['level'] == $chardata[$i - 1]['level']) {
                if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ES && $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS && $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS && $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN && $chardata[$i + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_AN;
                }
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ET) {
                if ($i > 0 && $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN && $chardata[$i]['level'] == $chardata[$i - 1]['level']) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                } else {
                    $j = $i + 1;
                    while ($j < $numchars && $chardata[$j]['level'] == $chardata[$i]['level']) {
                        if ($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                            break;
                        } elseif ($chardata[$j]['type'] != \Mpdf\Ucdn::BIDI_CLASS_ET) {
                            break;
                        }
                        ++$j;
                    }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ET || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ES || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS) {
                $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_ON;
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                if ($i == 0) {
                    // Start of Level run
                    if ($chardata[$i]['sor'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                        $chardata[$i]['type'] = $chardata[$i]['sor'];
                    }
                } else {
                    for ($j = $i - 1; $j >= 0; $j--) {
                        if ($chardata[$j]['level'] != $chardata[$i]['level']) {
                            // Level run boundary
                            if ($chardata[$j + 1]['sor'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                                $chardata[$i]['type'] = $chardata[$j + 1]['sor'];
                            }
                            break;
                        } elseif ($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_L;
                            break;
                        } elseif ($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                            break;
                        }
                    }
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) {
                $left = -1;
                // LEFT
                if ($i == 0) {
                    // first char
                    ...
                    // run boundary
                    $left = $chardata[$i]['sor'];
                } elseif ($chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                    $left = \Mpdf\Ucdn::BIDI_CLASS_L;
                } elseif ($chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$i - 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                    $left = \Mpdf\Ucdn::BIDI_CLASS_R;
                }
                // RIGHT
                $right = -1;
                $j = $i;
                // move to the right of any following neutrals OR hit a run boundary
                while (($chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$j]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) && $j <= $numchars - 1) {
                    if ($j == $numchars - 1) {
                        // last char
                        ...
                        $right = $chardata[$j]['eor'];
                        break;
                    } elseif ($chardata[$j + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                        $right = \Mpdf\Ucdn::BIDI_CLASS_L;
                        break;
                    } elseif ($chardata[$j + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$j + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$j + 1]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                        $right = \Mpdf\Ucdn::BIDI_CLASS_R;
                        break;
                    }
                    $j++;
                }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) {
                $chardata[$i]['type'] = $chardata[$i]['level'] % 2 ? \Mpdf\Ucdn::BIDI_CLASS_R : \Mpdf\Ucdn::BIDI_CLASS_L;
                $chardata[$i]['orig_type'] = $chardata[$i]['type'];
                // Need to store the original 'WS' for reference in L1 below
            }
        }
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            ...
            if ($odd) {
                if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['level'] += 1;
                }
            } else {
                if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                    $chardata[$i]['level'] += 1;
                } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                    $chardata[$i]['level'] += 2;
                }
            }
        ...
        for ($i = $numchars - 1; $i > 0; $i--) {
            if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS || isset($chardata[$i]['orig_type']) && $chardata[$i]['orig_type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) {
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($chardata[$i]['level'] >= $j) {
                    ...
                    if (isset(\Mpdf\Ucdn::$mirror_pairs[$chardata[$i]['char']]) && $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                        $chardata[$i]['char'] = \Mpdf\Ucdn::$mirror_pairs[$chardata[$i]['char']];
                    }
                    $revarr[] = $chardata[$i];
                } else {
                    if ($onlevel) {
        ...
        foreach ($chardata as $cd) {
            ...
            if ($cd['type'] == \Mpdf\Ucdn::BIDI_CLASS_L) {
                $rtl_content |= 1;
            } elseif ($cd['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                $rtl_content |= 2;
            }
            $cctr++;
        }
    ...
    public function bidiPrepare(&$para, $dir)
    {
                ...
                if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) {
                    ...
                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8296) {
                        ...
                        while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                            ...
                            if ($para[$nc2][18]['char_data'][$i2]['bidi_class'] === \Mpdf\Ucdn::BIDI_CLASS_L || $para[$nc2][18]['char_data'][$i2]['bidi_class'] == \Mpdf\Ucdn::BIDI_CLASS_AL || $para[$nc2][18]['char_data'][$i2]['bidi_class'] === \Mpdf\Ucdn::BIDI_CLASS_R) {
                        ...
                        if (!$fsi && $fsi !== 0) {
                            if ($pel == 1) {
                                $fsi = \Mpdf\Ucdn::BIDI_CLASS_R;
                            } else {
                                $fsi = \Mpdf\Ucdn::BIDI_CLASS_L;
                            }
                        }
                    }
                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $fsi === \Mpdf\Ucdn::BIDI_CLASS_L) {
                        // LRI or FSI-L
                        ...
                        $next_level = $cel + 2 - $cel % 2;
                    } elseif ($chunkOTLdata['char_data'][$i]['uni'] == 8295 || $fsi == \Mpdf\Ucdn::BIDI_CLASS_R || $fsi == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                        // RLI or FSI-R
                    ...
                    if ($dos != -1) {
                        ...
                        $chardir = $chunkOTLdata['char_data'][$i]['bidi_class'];
                        if ($chardir == \Mpdf\Ucdn::BIDI_CLASS_R || $chardir == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                            $strongrtl = \true;
                        }
                    }
                    $chunkOTLdata['char_data'][$i]['level'] = $cel;
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($firstchar || $level != $prelevel) {
                        $chardata[$i]['sor'] = \max($prelevel, $level) % 2 ? \Mpdf\Ucdn::BIDI_CLASS_R : \Mpdf\Ucdn::BIDI_CLASS_L;
                    }
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN && $laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                        $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_AN;
                    }
                    if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AL) {
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AL) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_R;
                }
            }
        }
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (!isset($chardata[$i]['sor']) && !isset($chardata[$i]['eor'])) {
                        if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ES && $prevtype == \Mpdf\Ucdn::BIDI_CLASS_EN && $nexttype == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                        } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS && $prevtype == \Mpdf\Ucdn::BIDI_CLASS_EN && $nexttype == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                        } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS && $prevtype == \Mpdf\Ucdn::BIDI_CLASS_AN && $nexttype == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_AN;
                        }
                    }
                    $prevtype = $chardata[$i]['type'];
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ET) {
                        if ($prevtype == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                            $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                            ...
                            while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                                ...
                                if ($nexttype == \Mpdf\Ucdn::BIDI_CLASS_EN) {
                                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_EN;
                                    break;
                                } elseif ($nexttype != \Mpdf\Ucdn::BIDI_CLASS_ET) {
                                    break;
                                }
                            }
                        }
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ET || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ES || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_CS)) {
                    $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_ON;
                }
            }
        }
        ...
        for ($ir = 0; $ir <= $dictr; $ir++) {
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if (isset($chardata[$i]['type']) && $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN && $laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_L) {
                        $chardata[$i]['type'] = \Mpdf\Ucdn::BIDI_CLASS_L;
                    }
                    if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AL)) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
            ...
            for ($nc = 0; $nc < $numchunks; $nc++) {
                ...
                for ($i = 0; $i < $numchars; ++$i) {
                    ...
                    if ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) {
                        $left = -1;
                        // LEFT
                        if ($laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_R || $laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_EN || $laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                            $left = \Mpdf\Ucdn::BIDI_CLASS_R;
                        } elseif ($laststrongtype == \Mpdf\Ucdn::BIDI_CLASS_L) {
                            $left = \Mpdf\Ucdn::BIDI_CLASS_L;
                        }
                        // RIGHT
                        $right = -1;
                        ...
                        if (isset($chardata[$i]['eor'])) {
                            ...
                            while (!($nc2 == $numchunks - 1 && $i2 == \count($para[$nc2][18]['char_data']) - 1)) {
                                ...
                                $nexttype = $para[$nc2][18]['char_data'][$i2]['type'];
                                if ($nexttype == \Mpdf\Ucdn::BIDI_CLASS_R || $nexttype == \Mpdf\Ucdn::BIDI_CLASS_EN || $nexttype == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                                    $right = \Mpdf\Ucdn::BIDI_CLASS_R;
                                    break;
                                } elseif ($nexttype == \Mpdf\Ucdn::BIDI_CLASS_L) {
                                    $right = \Mpdf\Ucdn::BIDI_CLASS_L;
                                    break;
                    ...
                    } elseif ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_L || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_EN || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_AN) {
                        $laststrongtype = $chardata[$i]['type'];
                    }
                }
            }
        ...
        for ($nc = 0; $nc < $numchunks; $nc++) {
            ...
            $numchars = \count($chardata);
            for ($i = 0; $i < $numchars; ++$i) {
                if (isset($chardata[$i]['type']) && ($chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_ON || $chardata[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS)) {
                    $chardata[$i]['orig_type'] = $chardata[$i]['type'];
                    // Need to store the original 'WS' for reference in L1 below
                    $chardata[$i]['type'] = $chardata[$i]['level'] % 2 ? \Mpdf\Ucdn::BIDI_CLASS_R : \Mpdf\Ucdn::BIDI_CLASS_L;
                }
    ...
    public function bidiReorder(&$chunkorder, &$content, &$cOTLdata, $blockdir)
    {
        ...
        for ($i = $numchars - 1; $i > 0; $i--) {
            if ($bidiData[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_WS || isset($bidiData[$i]['orig_type']) && $bidiData[$i]['orig_type'] == \Mpdf\Ucdn::BIDI_CLASS_WS) {
        ...
        for ($j = $maxlevel; $j > 0; $j--) {
            ...
            for ($i = 0; $i < $numchars; ++$i) {
                if ($bidiData[$i]['level'] >= $j) {
                    $onlevel = \true;
                    // L4. A character is depicted by a mirrored glyph if and only if (a) the resolved directionality of that character is R, and (b) the Bidi_Mirrored property value of that character is true.
                    if (isset(\Mpdf\Ucdn::$mirror_pairs[$bidiData[$i]['uni']]) && $bidiData[$i]['type'] == \Mpdf\Ucdn::BIDI_CLASS_R) {
                        $bidiData[$i]['uni'] = \Mpdf\Ucdn::$mirror_pairs[$bidiData[$i]['uni']];
                    }
                    $revarr[] = $bidiData[$i];
                } else {
                    if ($onlevel) {
    ...
    private function _getOTLscriptTag($ScriptLang, $scripttag, $scriptblock, $shaper, $useOTL, $mode)
    {
        ...
        */
        ...
        } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_CYRILLIC) {
            if (!($useOTL & 0x2)) {
                return ['', \false];
            }
        } elseif ($scriptblock == \Mpdf\Ucdn::SCRIPT_GREEK) {
            if (!($useOTL & 0x4)) {
                return ['', \false];
            }
        } elseif ($scriptblock >= \Mpdf\Ucdn::SCRIPT_HIRAGANA && $scriptblock <= \Mpdf\Ucdn::SCRIPT_YI) {
            if (!($useOTL & 0x8)) {
                return ['', \false];
            }
        } else {
    ...
    private function _getOTLLangTag($ietf, $available)
    {
        ...
        if ($lang != '' && isset(\Mpdf\Ucdn::$ot_languages[$lang])) {
            $langsys = \Mpdf\Ucdn::$ot_languages[$lang];
        } elseif ($lang != '' && $country != '' && isset(\Mpdf\Ucdn::$ot_languages[$lang . '' . $country])) {
            $langsys = \Mpdf\Ucdn::$ot_languages[$lang . '' . $country];
        } else {
            $langsys = "DFLT";
        }
